...
const fetchSchema = async (collection: string): Promise<DynamicFormStrapiSchema> => {
  ...
  if (!res.ok) {
    ...
    console.error('❌ Schema fetch failed:', { status: res.status, error: errorText });
  ...
  <T extends { id?: number }>(
    ...
    React.useEffect(() => {
      if (document && mergedDefaultValues) {
        console.log('🔄 Resetting form with new document data:', mergedDefaultValues);
    ...
    }, [formFactory.form.formState.isDirty, watchedValues, mergedDefaultValues, isCustomDirty, onDirtyChange]);
    ...
    const hasValidSchema = schema && (
      (schema.schema?.attributes && Object.keys(schema.schema.attributes).length > 0) ||
      (schema.attributes && Object.keys(schema.attributes).length > 0)
    );
    
    if (!hasValidSchema) {
      console.log('❌ Schema validation failed:', { schema, hasSchema: !!schema?.schema, hasAttributes: !!schema?.attributes });
      return <div style={{ color: "red" }}>Schema not found or empty for {collection}</div>;
    }
